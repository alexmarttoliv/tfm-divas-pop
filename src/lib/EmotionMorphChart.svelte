<script>
  ...
  // Progresso 0-1 ao longo de 2 steps
  $: morphProgress = Math.max(0, Math.min(1, (activeIndex - data.length + 1) / 2));
  
  // Opacidades separadas
  $: lineOpacity = 1 - morphProgress;
  $: ribbonOpacity = morphProgress;
  ...
  <div class="graphic" class:full={isRibbonMode}>
    <div bind:this={container} class="chart-container">
      <svg width={svgWidth} height={height}>
        ...
        <!-- LINE CHART (opacity controlada por lineOpacity) -->
        <g opacity={lineOpacity}>
          ...
          {#each [0, 0.2, 0.4, 0.6] as v}
            <g transform={`translate(${margin.left}, ${yLine(v)})`}>
              <line x2={svgWidth - margin.right} stroke="#eee" />
          ...
          {#each emotions as emotion}
            {#each data.slice(0, data.length - 1) as d, i}
              ...
              {#if i < activeIndex}
                <line
                  ...
                  y1={yLine(d[emotion.key])}
                  x2={decadeCenter(next.decade) - xBand.bandwidth() / 2}
                  y2={yLine(next[emotion.key])}
            ...
            {#if activeIndex >= 0 && activeIndex < data.length}
              ...
              <text
                ...
                y={yLine(currentDecade[emotion.key]) + 4}
                font-size="16"
                font-weight="600"
                fill={emotion.color}
                text-anchor="start"
        ...
        <!-- RIBBON (opacity controlada por ribbonOpacity) -->
        <g opacity={ribbonOpacity}>
          {#each ribbonSeries as serie (serie.key)}
            <path d={areaGenerator(serie.points)} fill={serie.color} opacity="0.9" />
          {/each}

          {#each indexedData as d}
            <text x={xRibbon(d.index)} y={height - margin.bottom + 28} text-anchor="middle" font-size="16" fill="#333">
              {d.decade}s
            </text>
          {/each}
        </g>
